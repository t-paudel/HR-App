...
public class LeaveManagementApiController implements LeaveManagementApi {
    ...
    public ResponseEntity<List<UserLeaves>> getAllEmployeeLeaveHistory(@ApiParam(value = "Employee id to delete",required=true) @PathVariable("employeeId") String employeeId) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<List<UserLeaves>>(objectMapper.readValue("[{'earnedLeave' : 1,'compOff' : 5,'employeeId' :'employeeId','maternityLeave' : 5,'displayStatus' : true,'id' : 'id',"
            + " 'lossOffPay' : 2,  'sickLeave' : 0,  'casualLeave' : 6}, "
                		+ "{'earnedLeave' : 1,'compOff' : 5,'employeeId' : 'employeeId','maternityLeave' : 5,'displayStatus' : true,'id' : 'id','lossOffPay' : 2,'sickLeave' : 0,'casualLeave' : 6"
                		} ]", 
